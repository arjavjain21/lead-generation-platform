...
def _valid_person_linkedin_url(url: Any) -> tuple[bool, str]:
    """Validate a person LinkedIn profile URL before it reaches POST /v2/enrichment/email.

    30-day telemetry: 215,898 HTTP 422s on /v2/enrichment/email came from
    malformed person URLs (company pages, non-LinkedIn hosts, whitespace-
    padded strings) reaching the endpoint. Every one is a wasted request, so
    they are stopped here at the client boundary for ALL call sites.

    Rules: must be a non-empty string; no whitespace anywhere; host is
    linkedin.com or any subdomain of it; path contains an "/in/" segment
    (person profile, not /company/ or /school/).

    Returns:
        (True, "") when the URL may be sent, else (False, reason) where the
        reason is safe to log on its own (it NEVER contains the URL itself).
    """
    ...
    if host != "linkedin.com" and not host.endswith(".linkedin.com"):
        return False, "host is not linkedin.com"
    if "/in/" not in (parsed.path or ""):
        return False, "path missing /in/ segment"
    return True, ""
...
async def find_work_email(
    ...
    Malformed person URLs (30d telemetry: 215,898 HTTP 422s) are rejected
    BEFORE the HTTP call and return the not-found shape at zero cost.
    """
    valid, reason = _valid_person_linkedin_url(person_linkedin_url)
    if not valid:
        # Log the REASON only — never the URL itself.
        logger.warning("blitz email skipped invalid linkedin url (%s)", reason)
        return {"found": False, "email": None, "all_emails": []}
    return await _post_with_retry(
        client,
...
async def person_enrich_by_linkedin(
    ...
    Malformed person URLs (30d telemetry: 215,898 HTTP 422s on this
    endpoint) are rejected BEFORE the HTTP call and return the not-found
    shape at zero cost.
    """
    valid, reason = _valid_person_linkedin_url(linkedin_url)
    if not valid:
        # Log the REASON only — never the URL itself.
        logger.warning("blitz email skipped invalid linkedin url (%s)", reason)
        return {"found": False, "email": None, "all_emails": []}
