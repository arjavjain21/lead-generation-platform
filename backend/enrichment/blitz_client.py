...
async def domain_emails(
    client: httpx.AsyncClient,
    domain: str,
    limit: int = 50,
) -> dict[str, Any]:
    """
    POST /v2/enrichment/domain-emails
    Get generic email patterns and verified emails for a domain.

    Note: This endpoint may not exist in Blitz API. If it returns 404,
    fall back to extracting emails from waterfall results.
    ...
    Returns:
        {
            "found": true,
            "domain": "acme.com",
            "email_pattern": "firstname.lastname@acme.com",
            "emails": [
    ...
    return await _post_with_retry(
        client,
        f"{BLITZ_BASE_URL}/v2/enrichment/domain-emails",
