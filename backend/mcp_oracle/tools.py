...
def explain_error_code(error_message: str) -> str:
    ...
    if "401" in msg_lower or "unauthorized" in msg_lower or "invalid token" in msg_lower:
        return (
            ...
            "- Invalid API key\n"
            "- Using an API key on the SSE stream "
            "(`GET /jobs/{job_id}/stream`) or on `/api/api-keys/*` / "
            "`/api/phone-enrichment/*` — those are JWT-only\n\n"
            "**Fixes:**\n"
            "- Every other `/api/enrichment/*` endpoint accepts `X-API-Key: lgp_...` "
            "(or the key as a Bearer token)\n"
            "- SSE stream: use a JWT (or `?token=`), or simply poll "
            "`GET /jobs/{job_id}` with your API key instead\n"
            "- Refresh expired JWT: `POST /api/auth/refresh`\n"
            "- Generate new API key: Account → API Keys in the UI (JWT required — "
...
def get_quota_guide() -> str:
    ...
    return (
        ...
        "- **Scraper only** (checked at estimate/job-create on the scraper paths): "
        "job creation is the heavy item — each task costs ~10 units\n"
        "- **Enrichment endpoints are NOT metered by this quota** — the real cost "
        "meter for enrichment is paid-provider spend behind the cascade "
        "(Blitz/SmartProspect/WizLeads/BetterEnrich credits). Control it with "
        "`selected_providers: [\"contacts_db\"]` (free) or `website_only: true` "
        "(Flow 1, zero paid providers)\n\n"
