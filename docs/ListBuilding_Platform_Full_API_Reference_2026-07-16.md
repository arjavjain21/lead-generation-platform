...
> **UI (2026-09-17):** this flow powers the **"Find Companies (TAM)"** page in the web UI
> (nav slot of the former Company Search). The page builds the same request from plain
> filters (company: name/industry/size/type/country; people: titles, seniority, min matches;
> cap; optional chain-enrich) and monitors the run on the Jobs page. **Every company the
> flow returns is automatically upserted into the contacts database** (domain-keyed business
> records, best-effort — a backup failure never fails the TAM job; summary counters:
> `companies_backed_up` / `companies_skipped_no_domain` / `companies_backup_failed`).
...
- Progress events (SSE / job events): one per page — `{"stage": "tam_page", "page": N, "companies_found": N, "has_next_page": bool, "message": "..."}`. `processed` tracks pages fetched. Lookalike runs add `{"stage": "tam_fanout", "variant": i, "variants": n, "message": "fan-out query i/n: industry=X"}` per query variant and a final `{"stage": "tam_ranked", ...}` when the scored export is written.
- The CSV is written incrementally (header + per-page flush/fsync), so it is live-downloadable while running, and a cancel between pages keeps completed pages (`status='partial'`). A live cursor + row count is persisted per page for crash diagnostics. **Exception — ranked (lookalike) runs:** rows buffer in memory and the CSV is written once at the end (scored + sorted); a cancel still writes the scored partial.
...
- **Output CSV columns (21, in order):** `name, domain, website, linkedin_url, industry, type, size, employees_on_linkedin, followers, founded_year, hq_city, hq_state, hq_country_code, hq_region, revenue, slogan, employee_growth_1y, matched_people, source, match_score, why_matched`. `domain` is nullable in the Blitz API — never assume it. The trailing `match_score` (int 0-100) / `why_matched` (e.g. `"prediction markets · industry match · size 51-200"`) columns are populated only by ranked lookalike runs and empty otherwise.

#### Lookalike 2.0 — ranked mode (2026-09-18)

When the request carries `seed_companies` (3–5 example companies: domains or LinkedIn company URLs) **without** `analyze_seeds`, the flow switches to deterministic ranked lookalikes (no LLM, no embeddings):

1. **Seed profiling** — each seed is resolved via Blitz (`domain_to_linkedin` → company enrich, GetLeads as fallback) and its homepage text fetched (`https://domain/`, `www.` fallback, 8s timeout, never fails the run).
2. **Query plan fan-out** — instead of one majority-industry query, retrieval runs **one Blitz query per distinct seed industry plus one keywords-only query** (niche keywords = tokens shared by ≥ 2 seed homepages minus a generic stopword list, plus any explicit `keywords_include`). Each variant is capped at 300 companies with the budget split so no variant starves another; total candidates cap at `LOOKALIKE_MAX_CANDIDATES` (default 2000). The GetLeads leg (when `sources` includes it) fans out the same way, still capped at `GETLEADS_LOOKALIKE_MAX_CREDITS`.
3. **Scoring** — every candidate is scored `100 × (0.5·trigram_text_similarity + 0.2·industry_match + 0.15·size_proximity + 0.15·keyword_hits)` against the seed profiles, sorted best-match first, and exported with `match_score` / `why_matched`.
4. **Backstop** — if the seeds resolve but carry neither industries nor keywords anywhere, the run is refused with the "random mix" 400 (previously this fired for any mixed-industry seed set; the fan-out plan now handles those).

`analyze_seeds: true` returns the profile without creating a job; the profile's `keywords` now include homepage-derived niche keywords and carries a `website_text_sha` provenance fingerprint. The job's `display_name` gains `· ranked`.
...
| 2026-09-18 | **Lookalike 2.0 — deterministic ranked lookalikes** on `POST /api/enrichment/flows/tam` (see I.1 "Lookalike 2.0"): seed runs now fan out one Blitz query per distinct seed industry + one keywords-only query (homepage-text niche keywords; `LOOKALIKE_MAX_CANDIDATES` 2000 / 300-per-variant caps), score every candidate (trigram text similarity + industry + size + keyword hits), and export sorted best-match-first with new trailing CSV columns `match_score` / `why_matched` (contract now 21 columns). Mixed-industry seeds (e.g. prediction-market apps) no longer 400 at the "random mix" guard — it remains as a backstop when nothing targetable survives. `analyze_seeds` profile gains homepage-derived keywords + `website_text_sha`. |
